...
!
!git $Id$
!svn $Id: def_avg.F 1210 2024-01-03 22:03:03Z arango $
!================================================== Hernan G. Arango ===
...
!
      DEFINE : IF (ldef) THEN
        ...
        IF (FoundError(exit_flag, NoError, 195, MyFile)) THEN
...
!
     ...
     &                 N(ng), DimIDs( 9))
        IF (FoundError(exit_flag, NoError, 281, MyFile)) RETURN
...
!
     ...
     &                   NF_FOUT, nvd3, u2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 575, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd4, v3dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 810, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                     NF_FOUT, nvd4, t3dgrd, Aval, Vinfo, ncname)
            IF (FoundError(exit_flag, NoError, 936, MyFile)) RETURN
          END IF
        END DO
!
...
!
     ...
     &                   NF_FOUT, nvd4, p3dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1104, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                   NF_FOUT, nvd3, u2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1143, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                     nvd4, u3dgrd, Aval, Vinfo, ncname)
            IF (FoundError(exit_flag, NoError, 1302, MyFile)) RETURN
          END IF
!
...
!
     ...
     &                   NF_FOUT, nvd3, u2dgrd, Aval, Vinfo, ncname)
          IF (FoundError(exit_flag, NoError, 1776, MyFile)) RETURN
        END IF
!
...
!
        ...
        IF (FoundError(exit_flag, NoError, 1870, MyFile)) THEN
...
!
     ...
     &                         ncid = AVG(ng)%ncid)
        IF (FoundError(exit_flag, NoError, 1879, MyFile)) RETURN
!
!  Inquire about the variables.
!
        CALL netcdf_inq_var (ng, model, ncname,                         &
     &                       ncid = AVG(ng)%ncid)
        IF (FoundError(exit_flag, NoError, 1885, MyFile)) RETURN
!
