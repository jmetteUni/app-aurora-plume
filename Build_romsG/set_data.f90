...
!
!git $Id$
!svn $Id: set_data.F 1210 2024-01-03 22:03:03Z arango $
!================================================== Hernan G. Arango ===
...
!
      ...
      UBij=BOUNDS(ng)%UBij
!
      CALL wclock_on (ng, iNLM, 4, 32, MyFile)
      CALL set_data_tile (ng, tile,                                     &
     &                    LBi, UBi, LBj, UBj,                           &
     &                    IminS, ImaxS, JminS, JmaxS)
      CALL wclock_off (ng, iNLM, 4, 38, MyFile)
!
      RETURN
...
!
     ...
     &                      update)
            IF (FoundError(exit_flag, NoError, 922, MyFile)) RETURN
          END IF
        END IF
      END IF
!
...
!
     ...
     &                      update)
            IF (FoundError(exit_flag, NoError, 1018, MyFile)) RETURN
          END IF
!
...
!
     ...
     &                      update)
            IF (FoundError(exit_flag, NoError, 1036, MyFile)) RETURN
          END IF
!
...
!
     ...
     &                      BOUNDARY(ng) % v_east,                      &
     &                      update)
            IF (FoundError(exit_flag, NoError, 1129, MyFile)) RETURN
          END IF
!
...
!
     ...
     &                       CLIMA(ng)%ubarclm,                         &
     &                       update)
        IF (FoundError(exit_flag, NoError, 1254, MyFile)) RETURN
!
     ...
     &                       CLIMA(ng)%vbarclm,                         &
     &                       update)
        IF (FoundError(exit_flag, NoError, 1261, MyFile)) RETURN
      END IF
!
