...
!
!git $Id$
!svn $Id: get_data.F 1210 2024-01-03 22:03:03Z arango $
!================================================== Hernan G. Arango ===
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, iNLM, 3, 91, MyFile)
!
!=======================================================================
...
!
     ...
     &                    BOUNDARY(ng) % vbarG_west)
          IF (FoundError(exit_flag, NoError, 990, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                    BOUNDARY(ng) % uG_west)
          IF (FoundError(exit_flag, NoError, 1089, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                    BOUNDARY(ng) % uG_south)
          IF (FoundError(exit_flag, NoError, 1141, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                    BOUNDARY(ng) % uG_north)
          IF (FoundError(exit_flag, NoError, 1167, MyFile)) RETURN
        END IF
!
...
!
     ...
     &                      BOUNDARY(ng) % tG_north(:,:,:,i))
            IF (FoundError(exit_flag, NoError, 1246, MyFile)) RETURN
          END IF
        END DO
      END IF
!
...
!
     ...
     &                  LBi, UBi, LBj, UBj, 2, 1,                       &
     &                  CLIMA(ng) % sshG)
        IF (FoundError(exit_flag, NoError, 1272, MyFile)) RETURN
      END IF
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, iNLM, 3, 1566, MyFile)
!
      RETURN
