...
!
!git $Id$
!svn $Id: yaml_parser.F 1210 2024-01-03 22:03:03Z arango $
!================================================== Hernan G. Arango ===
...
!
      CALL self%create ()
      IF (yaml_Error(yaml_ErrFlag, NoErr, 355, MyFile)) THEN
        status=yaml_ErrFlag
        RETURN
      END IF
!
...
!
     ...
     &      STATUS='old', IOSTAT=io_err, IOMSG=io_errmsg)
      IF (io_err.ne.0) THEN
        yaml_ErrFlag=5
        IF (yaml_Error(yaml_ErrFlag, NoErr, 411, MyFile)) THEN
...
!
        SELECT CASE (status)
          CASE (-1)       ! error condition during reading
            yaml_ErrFlag=5
            IF (yaml_Error(yaml_ErrFlag, NoErr, 434, MyFile)) THEN
...
!
        IF (Nblanks.gt.0) THEN
          IF (MOD(Nblanks, self%indent).ne.0) THEN
            yaml_ErrFlag=2
            IF (yaml_Error(yaml_ErrFlag, NoErr, 466, MyFile)) THEN
...
!
      ...
      ELSE
        yaml_ErrFlag=7
        status=yaml_ErrFlag
        IF (yaml_Error(yaml_ErrFlag, NoErr, 780, MyFile)) THEN
