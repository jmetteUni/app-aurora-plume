...
!
     ...
     &                     GRID(ng) % z_v,                              &
     &                     SetFillVal = .FALSE.)
        IF (FoundError(status, nf90_noerr, 414, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     GRID(ng) % z_w,                              &
     &                     SetFillVal = .FALSE.)
        IF (FoundError(status, nf90_noerr, 438, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                       OCEAN(ng) % t(:,:,:,nrhs(ng),itrc))
          IF (FoundError(status, nf90_noerr, 1119, MyFile)) THEN
            IF (Master) THEN
...
!
     ...
     &                     MIXING(ng) % Akv,                            &
     &                     SetFillVal = .FALSE.)
        IF (FoundError(status, nf90_noerr, 1273, MyFile)) THEN
          IF (Master) THEN
...
!
     ...
     &                     MIXING(ng) % Akt(:,:,:,itemp),               &
     &                     SetFillVal = .FALSE.)
        IF (FoundError(status, nf90_noerr, 1297, MyFile)) THEN
          IF (Master) THEN
