...
!
!git $Id$
!svn $Id: check_multifile.F 1210 2024-01-03 22:03:03Z arango $
!================================================== Hernan G. Arango ===
...
!
     ...
     &                     Tmin, Tmax, Tscale,  Lcheck)
        IF (FoundError(exit_flag, NoError, 264, MyFile)) RETURN
...
!
      ...
      END SELECT
      IF (FoundError(exit_flag, NoError, 630, MyFile)) THEN
...
!
     ...
     &                          start = (/1/),                          &
     &                          total = (/1/))
          IF (FoundError(exit_flag, NoError, 742, MyFile)) RETURN
!
          CALL netcdf_get_time (ng, model, ncname, TvarName,            &
...
!
      ...
      END SELECT
      IF (FoundError(exit_flag, NoError, 780, MyFile)) RETURN
!
